...
1. **Step 1 **의제별 마디 나누기 (No Skipping)**: 텍스트를 시작부터 끝까지 촘촘히 분석해. 특히 '전환어(자, 다음은, 그런데, 하지만 등)'를 기준으로 구간을 나눠
2. **Step 2 (논리적 마디 분할)**: 주제가 바뀌는 지점을 찾되, 이전 구간과 다음 구간 사이의 시간 공백이 너무 크지 않도록(최대 3~5분 내외) 중간의 논의 과정도 충실히 포함하여 세분화한다.
...
# Strict Rules
1. **No Time Gap (시간 누락 금지)**: 핵심 결론으로 바로 점프하지 마라. 결론에 도달하기까지의 '과정(질의, 답변, 반박)'도 중요한 데이터다. 타임라인이 3분 이상 비어있다면 해당 구간의 내용을 다시 찾아 노드를 추가하라.
