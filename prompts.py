...
# 파일 상단 부분 수정
from get_transcriptions import get_simple_transcriptions
...
1. **Step 1 **의제별 마디 나누기 (No Skipping)**: 텍스트를 시작부터 끝까지 촘촘히 분석해. 특히 '전환어(자, 다음은, 그런데, 하지만 등)'를 기준으로 인덱스를 나누되, 한 인덱스가 5분(300초)을 넘지 않도록 세분화해.
...
# Strict Rules
1. **No Time Gap (시간 누락 금지)**: 핵심 결론(결의안 등)으로 바로 점프하지 마라. 결론에 도달하기까지의 '과정(질의, 답변, 반박)'도 중요한 데이터다. 타임라인이 5분 이상 비어있다면 해당 구간의 내용을 다시 찾아 인덱스를 추가하라.
2. **정보 보존**: 요약을 위해 구체적 근거(법안 번호, 통계 등)를 생략하지 마라.
3. **교정 (필수)**: 맥럭상 오타로 추정되는 것은 반드시 수정하라. (예시: 신호적->실효적 / 영상연담->영사면담 / 영동->영토 / 지옥화->분쟁 지역화 / 적반된->석방된)
...
# 출력 형식 (JSON Only, 한국어로 출력)
{
  "keywords": [...],
  "overview": "전체 타임라인을 관통하는 핵심 주장과 서사적 흐름 요약",
  "reconstruction": [
    {
      "index": "1",
      "content": "공백 없이 이어지는 서사적 헤드라인",
      "start": 00.0,
      "subitems": [
        {
          "index": "1_1",
          "content": "해당 시간대의 구체적 발언과 팩트.",
          "start": 00.0
        }
      ]
    }
